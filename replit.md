...
- **Staff Assignment to Properties**: Platform Admins (ADMIN) and Organization Admins (ORG_ADMIN) can assign Operations Managers (OPS role) and Support Workers (SUPPORT role) to properties during property creation. The StaffPicker component provides a multi-select interface for choosing staff members from the organization. Assignments are stored with start/end dates and displayed on the property detail page grouped by role (Operations Managers and Support Workers sections). Each assignment shows the staff member's name, email, role badge, active status, and assignment date.
